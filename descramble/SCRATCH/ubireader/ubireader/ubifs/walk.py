...
def index(ubifs, lnum, offset, inodes={}, bad_blocks=[]):
    ...
    print(index , '%s file addr: %s' % (chdr, ubifs.file.last_read_addr()))
    ...
    print('read bytes in node_buf', len(node_buf))
    ...
    if chdr.node_type == UBIFS_IDX_NODE:
        ...
        print(index, '%s file addr: %s' % (idxn, file_offset))
        verbose_display(idxn)
        branch_idx = 0

        for branch in idxn.branches:
            verbose_log(index, '-------------------')
            print(index, '%s file addr: %s' % (branch, file_offset + UBIFS_IDX_NODE_SZ + (branch_idx * UBIFS_BRANCH_SZ)))
        ...
        print(index, '%s file addr: %s, ino num: %s' % (inon, file_offset, ino_num))
        ...
        print(index, '%s file addr: %s, ino num: %s' % (datn, file_offset, ino_num))
        ...
        print(index, '%s file addr: %s, ino num: %s' % (dn, file_offset, ino_num))
        verbose_display(dn)
        ...
        print('not handling node type:', chdr.node_type)
